...
namespace STGP_Sharp.Utilities.GeneralCSharp
{
    public static class Extensions
    {
        public static Vector3 Centroid(this IEnumerable<Vector3> vectors)
        {
            ...
            if (i == 0)
            {
                return new Vector3(new Vector2(0, 0), 0);
            }
        ...
        public static T GetRandomEntry<T>(this T[] a, Random rand)
        {
            if (a.Length == 0)
            {
                throw new Exception("Getting random entry from zero-length array");
            }
        ...
        {
            using IEnumerator<TSrc> e = ie.GetEnumerator();
            if (!e.MoveNext())
            {
                return -1;
            }
        ...
        {
            var index = 0;
            foreach (TSrc guy in ie)
            {
                if (matchMe.Equals(guy))
                {
                    return index;
                }
        ...
        {
            foreach (TParentType parent in parents)
            {
